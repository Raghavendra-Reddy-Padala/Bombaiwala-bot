...
const handleMessage = async (sender, msgType, message) => {
    ...
    if (msgType === 'location') {
        ...
        await sendReply(sender, '📍 Thanks for the location! But we don\'t need it right now.\nType *hi* to start ordering.');
        return;
    }
...
const handleName = async (sender, text, session) => {
    if (text.length < 2) {
        await sendReply(sender, '⚠️ Please enter your name (at least 2 characters).');
        return;
    }
...
const handleLocation = async (sender, message, session) => {
    ...
    if (!lat || !lng) {
        await sendReply(sender, '⚠️ Could not read your location. Please try sharing again.');
        return;
    }
